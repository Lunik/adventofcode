  ...
  for instruction in instructions:
    for interation in range(instruction[1]):
      ...
      for snake_part in range(1, len(snake)):
        ...
        if max(abs_dist) > 1:
          if dist[0] == 0:
            snake[snake_part] = sum_tuple(snake[snake_part], (0, -1 if dist[1] < 0 else 1))
          elif dist[1] == 0:
            snake[snake_part] = sum_tuple(snake[snake_part], (-1 if dist[0] < 0 else 1, 0))
          else:
            snake[snake_part] = sum_tuple(snake[snake_part], (-1 if dist[0] < 0 else 1, -1 if dist[1] < 0 else 1))
