...
def parse(data):
  ...
  for line in data:
    if line == '':
      ...
      new_grid.append(
        [ {'found': False, 'value': int(number)} for number in list(filter(None, line.split(' ')))]
      )
...
def check_number(grid, number):
  for line in grid:
    for current_number in filter(lambda n: (not n['found']) and (n['value'] == number), line):
...
def calculate_solution(winning_number, orientation, grid):
  ...
  first_sum = reduce(
    ...
    map(
      ...
      reduce(
        lambda a,b: a + b,
        map(lambda n: n['value'] if not n['found'] else 0, line)), grid))
