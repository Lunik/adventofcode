...
def find_neighbors(position, dimension):
  ...
  for pos_z in range(-1, 2):
    if position[0] + pos_z not in range(len(dimension)):
      continue
    for pos_y in range(-1, 2):
      if position[1] + pos_y not in range(len(dimension[pos_z])):
        continue
      for pos_x in range(-1, 2):
        if position[2] + pos_x not in range(len(dimension[pos_z][pos_y])):
          continue

        if pos_z == pos_y == pos_x == 0:
          continue

        neighbors.append(dimension[position[0] + pos_z][position[1] + pos_y][position[2] + pos_x])

  return neighbors


def generate_dimension(size):
  return [[['.' for _ in range(size[0])] for _ in range(size[1])] for _ in range(size[2])]
  ...
  for pos_z, dimension_z in enumerate(dimension):
    for pos_y, dimension_y in enumerate(dimension_z):
      for pos_x, _ in enumerate(dimension_y):
        neighbors = find_neighbors((pos_z, pos_y, pos_x), dimension)
        active_neighbors = neighbors.count('#')

        if dimension[pos_z][pos_y][pos_x] == "#":
          if active_neighbors not in [2, 3]:
            new_dimension[pos_z][pos_y][pos_x] = '.'
          else:
            new_dimension[pos_z][pos_y][pos_x] = dimension[pos_z][pos_y][pos_x]
...
def count_active_cube(dimension):
  ...
  for _, dimension_z in enumerate(dimension):
    for _, dimension_y in enumerate(dimension_z):
