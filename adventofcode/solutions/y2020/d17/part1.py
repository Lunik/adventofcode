    ...
    for pos_z in range(-1, 2):
        ...
        for pos_y in range(-1, 2):
            ...
            for pos_x in range(-1, 2):
                ...
                neighbors.append(
                    dimension[position[0] + pos_z][position[1] + pos_y][
                        position[2] + pos_x
                    ]
                )
...
def generate_dimension(size):
    return [
        [["." for _ in range(size[0])] for _ in range(size[1])] for _ in range(size[2])
    ]
            ...
            for pos_x, _ in enumerate(dimension_y):
                ...
                if dimension[pos_z][pos_y][pos_x] == "#":
                    if active_neighbors not in [2, 3]:
                        ...
                        new_dimension[pos_z][pos_y][pos_x] = dimension[pos_z][pos_y][
                            pos_x
                        ]
