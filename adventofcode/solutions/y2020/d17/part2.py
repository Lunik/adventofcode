        ...
        for pos_z in range(-1, 2):
            ...
            for pos_y in range(-1, 2):
                ...
                for pos_x in range(-1, 2):
                    if position[3] + pos_x not in range(
                        len(dimension[pos_w][pos_z][pos_y])
                    ):
                        continue
                    ...
                    neighbors.append(
                        dimension[position[0] + pos_w][position[1] + pos_z][
                            position[2] + pos_y
                        ][position[3] + pos_x]
                    )
...
def generate_dimension(size):
    return [
        [
            [["." for _ in range(size[3])] for _ in range(size[2])]
                ...
                for pos_x, _ in enumerate(dimension_y):
                    ...
                    if dimension[pos_w][pos_z][pos_y][pos_x] == "#":
                        if active_neighbors not in [2, 3]:
                            ...
                            new_dimension[pos_w][pos_z][pos_y][pos_x] = dimension[
                                pos_w
                            ][pos_z][pos_y][pos_x]
