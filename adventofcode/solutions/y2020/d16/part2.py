...
def main():
    ...
    valid_passports = list(
        filter(lambda p: passport_error_rate(rules, p) == 0, passports)
    )
    ...
    departure_attributes = list(
        filter(lambda a: "departure" in a, possible_attribute_positions.keys())
    )

    positions = {}

    while len(positions.keys()) < len(rules.keys()):
        for attribute in set(possible_attribute_positions.keys()) ^ set(
            positions.keys()
        ):
            attr_positions = set(possible_attribute_positions[attribute]) ^ set(
                positions.values()
            )
        ...
        found_departure_attributes = list(
            filter(lambda a: "departure" in a, positions.keys())
        )
