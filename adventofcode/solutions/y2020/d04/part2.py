...
def check_hgt(value):
    ...
    return len(match_groups) == 2 and (
        check_range([150, 193], match_groups[0])
...
def check_pid(value):
    return len(regex_groups("(\\d{9})", value)) == 1 and (len(str(value)) == 9)
...
def is_valid_extended(passport):
    return reduce(
        lambda a, b: a and b, map(lambda key: checkDict[key](passport[key]), passport)
    )
...
def verify(required_attr, ignore_attr, passports):
    ...
    for passport in passports:
        if is_valid(required_attr, ignore_attr, passport) and is_valid_extended(
            passport
        ):
